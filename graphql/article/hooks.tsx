...
export const useGetArticle = (slugAndId) =>
  useQuery(GET_ARTICLE, { variables: { slugAndId } });
...
export const useUpdateArticle = () => {
  const [updateArticleMutation] = useMutation(UPDATE_ARTICLE, {
    update: (cache: any, { data: { updateArticle } }) => {
      const { variables } = cache.watches.values().next().value;
      const { articles } = cache.readQuery({ query: GET_ARTICLES, variables });
      cache.writeQuery({
        query: GET_ARTICLES,
        variables,
        data: {
          articles: articles.map((articleItem) =>
            articleItem.id !== updateArticle.id ? articleItem : updateArticle
          ),
        },
      });
    },
...
export const useDeleteArticle = () => {
  const [deleteArticleMutation] = useMutation(DELETE_ARTICLE, {
    update: (cache: any, { data: { deleteArticle } }) => {
      const { variables } = cache.watches.values().next().value;
      const { articles } = cache.readQuery({ query: GET_ARTICLES, variables });
      cache.writeQuery({
        query: GET_ARTICLES,
        ...
        data: {
          articles: articles.filter(
            (articleItem) => articleItem.id !== deleteArticle.id
          ),
        },
      });
    },
